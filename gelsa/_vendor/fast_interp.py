"""
Vendored copy of fast_interp by David Stein.

Source:  https://github.com/dbstein/fast_interp (fast_interp/fast_interp.py)
Commit:  e4bcf89b19b302492dab3227c6eb02786aa25c5b (2022-02-18)

Copyright 2019 David Stein

Licensed under the Apache License, Version 2.0 (the "License"); you may not
use this file except in compliance with the License. A copy of the License is
included in LICENSE.fast_interp in this directory, or at
...
Modifications by the gelsa developers:
  - Enabled numba on-disk caching (cache=True) for all jitted functions.
  - The parallel variants built by pjit() are given a distinct __qualname__
    so they do not share numba cache entries with the serial variants.
"""
import types
import numpy as np
import numba
...
def pjit(func):
    # numba's cache key ignores the parallel flag, so give the parallel copy a
    # distinct qualname to keep it from sharing cache entries with the serial one
    pfunc = types.FunctionType(func.__code__, func.__globals__, func.__name__,
                               func.__defaults__, func.__closure__)
    pfunc.__qualname__ = func.__qualname__ + '_parallel'
    return numba.njit(pfunc, parallel=True, cache=True)
