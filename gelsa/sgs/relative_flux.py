...
class RelativeFluxCalibration:
    ...
    def _get_interpolators(self, data, dq, wstart, wstep):
        ...
        dq[np.isnan(dq)] = 1.0
        corr_interp = RegularGridInterpolator((ZZ, YY, XX), data,
                                              bounds_error=False,
                                              fill_value=None)
        dq_interp = RegularGridInterpolator((ZZ, YY, XX), dq,
                                            bounds_error=False,
                                            fill_value=None)
        limits = (
            (ZZ.min(), ZZ.max()),
...
def get_flux_loss_fov(detector_model, pack, xfov, yfov, wavelength_ang=15000):
    ...
    coords = np.transpose([wavelength_ang[sel], yfov_s[sel], xfov_s[sel]])

    mag = pack['Map'](coords)

    fluxloss = np.zeros(len(xfov), dtype='d')
...
def get_flux_loss(detector_model, pack, x, y, det_id, wavelength_ang=15000):
    ...
    xfov, yfov = detector_model.getFOVPosition(x, y, det_id)
    xfov_s, yfov_s = detector_model.getScaledFOV(xfov, yfov)
